...
BOOL xMBPortSerialInit( UCHAR ucPORT, ULONG ulBaudRate, UCHAR ucDataBits, eMBParity eParity ) {
    ...
    USART_Config( pMbPort, (uint32_t)ulBaudRate, (uint32_t)ucDataBits, (uint32_t)eParity, (uint8_t)STOPBITS_1 );
...
BOOL xMBPortSerialPutByte( CHAR ucByte ) {
    LL_USART_TransmitData8(pMbPort->handle, ucByte);
    return TRUE;
}

BOOL xMBPortSerialGetByte( CHAR * pucByte ) {
    *pucByte = LL_USART_ReceiveData8(pMbPort->handle);
    return TRUE;
}

#endif    //defined(MODBUS_PORT)
